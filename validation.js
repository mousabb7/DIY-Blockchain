...
const isValidTransaction = transaction => {
  ...
  const toSign = transaction.source + transaction.recipient + transaction.amount;
...
const isValidChain = blockchain => {
  ...
  return blocks.map(b => b.transactions).reduce((flat, txns) => flat.concat(txns), []).every(isValidTransaction);
};
...
const breakChain = blockchain => {
  // Your code here
  blockchain.blocks[1].transactions[0].amount = 1000000000000000000000000000;
};
